...
class LLMProvider:
    ...
    def generate_studio_artifact(self, artifact_type: str, context_chunks: list) -> str:
        ...
        if artifact_type == "briefing_doc":
            ...
            p_q1 = max(1, max_pg // 4)
            p_q2 = max(2, max_pg // 2)
            p_q3 = max(3, (max_pg * 3) // 4)

            overview_sents = [s for s in sentences if s['page'] <= p_q1] or sentences[:5]
            key_points_sents = [s for s in sentences if p_q1 < s['page'] <= p_q2] or sentences[5:15]
            deep_analysis_sents = [s for s in sentences if p_q2 < s['page'] <= p_q3] or sentences[15:25]
            conclusion_sents = [s for s in sentences if s['page'] > p_q3] or sentences[-5:]
            ...
            return (
                ...
                f"**Source Coverage**: {len(sentences)} key statements analyzed across full document (Pages 1–{max_pg})\n\n"
                f"---\n\n"
                f"## 1. Executive Summary & Synthesis (Pages 1–{p_q1})\n"
                f"{overview_text}\n\n"
                f"## 2. Core Concepts & Operational Principles (Pages {p_q1+1}–{p_q2})\n"
                f"{key_points_formatted}\n\n"
                f"## 3. Deep Methodological & Technical Analysis (Pages {p_q2+1}–{p_q3})\n"
                f"{deep_analysis_formatted}\n\n"
                f"## 4. Key Conclusions & Strategic Takeaways (Pages {p_q3+1}–{max_pg})\n"
